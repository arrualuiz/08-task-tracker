...
function render(){
  ...
  for(const [key,name] of Object.entries(names)){
    ...
    for(const item of items){
      ...
      card.append(node('h3',item.title),node('small',`${item.kind==='block'?'Reserva de tempo · ':''}${item.time||'Sem horário'}${item.duration?` · ${item.duration} min`:''}${item.account?` · ${item.account}`:''}`));
  ...
  $('summary').textContent=`${plan.items.length} itens · ${plan.items.filter(i=>i.done).length} concluídos · ${plan.items.reduce((sum,i)=>sum+i.duration,0)} min planejados`;
  suggestions();
}
/** Afinidade simples com a data; não expande ICS nem afirma que seja ocorrência confirmada. */
function fits(r){const d=new Date(`${plan.date}T12:00:00`),day=['SU','MO','TU','WE','TH','FR','SA'][d.getDay()],p=r.recurrence;if(r.date>plan.date)return false;return p.frequency==='ONCE'?r.date===plan.date:p.frequency==='DAILY'?true:p.frequency==='WEEKLY'?p.days.includes(day):p.monthlyMode==='date'?p.monthDay===d.getDate():p.weekday===day&&p.ordinal===Math.ceil(d.getDate()/7);}
function suggestions(){
  const target=$('suggestions');target.replaceChildren();const query=$('search').value.toLocaleLowerCase('pt-BR');
  const list=routines.filter(r=>r.status!=='paused'&&r.kind!=='info'&&!plan.items.some(i=>i.routineId===r.id)&&(!$('account').value||r.account===$('account').value)&&r.title.toLocaleLowerCase('pt-BR').includes(query)&&($('suggestion-mode').value==='all'||fits(r))).sort((a,b)=>(a.status==='approved'?0:1)-(b.status==='approved'?0:1)||a.time.localeCompare(b.time)||a.title.localeCompare(b.title));
  if(!list.length)target.append(node('p','Nenhuma sugestão neste filtro. Você pode adicionar uma tarefa livre.'));
  for(const r of list){const card=node('div','','suggestion');card.append(node('strong',r.title),node('small',`${r.status==='approved'?'Rotina aprovada':'Ainda para revisar'} · ${r.time||'Sem horário'} · ${r.account}`),button('+ Escolher para o dia',()=>{edit({id:'',routineId:r.id,title:r.title,account:r.account,notes:r.notes,time:r.time,duration:Math.min(r.duration,1440),block:r.time?(r.time<'12:00'?'morning':r.time<'18:00'?'afternoon':'evening'):'anytime',kind:'task',done:false});}));target.append(card);}
}
let editingSource=null;
function edit(item,block='anytime',kind='task'){
  if(!plan||busy)return;editingSource=item;const f=$('item-form');f.reset();const values=item||{id:'',title:'',block,kind,time:'',duration:kind==='block'?60:0,notes:'',account:''};for(const key of ['id','title','block','kind','time','duration','notes','account'])f.elements.namedItem(key).value=values[key];$('dialog-title').textContent=item?.id?'Editar item':kind==='block'?'Reservar tempo':'Adicionar ao dia';$('form-error').textContent='';$('editor').showModal();f.elements.namedItem('title').focus();
}
$('item-form').onsubmit=e=>{
  e.preventDefault();const f=new FormData(e.target),item={id:f.get('id')||crypto.randomUUID(),routineId:editingSource?.routineId||null,title:f.get('title').trim(),block:f.get('block'),kind:f.get('kind'),time:f.get('time'),duration:Number(f.get('duration')),notes:f.get('notes'),account:f.get('account'),done:editingSource?.done||false};
...
$('save').onclick=async()=>{
  ...
  // Impede editar o rascunho enquanto a resposta do salvamento está em trânsito.
...
$('reload').onclick=load;$('date').onchange=load;for(const id of ['search','account','suggestion-mode'])$(id).oninput=()=>{if(plan)suggestions();};
window.addEventListener('beforeunload',e=>{if(dirty||busy){e.preventDefault();e.returnValue='';}});$('date').value=today();load();
